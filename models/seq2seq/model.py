...
class EncoderRNN(BaseRNN):
    ...
    def forward(self, input_var, input_lengths=None):
        ...
        """
        srcs_ids_pad = input_var[0]
        input_lengths = input_var[1]
        embedded = self.embedding(srcs_ids_pad)
...
def build(hidden_size, batch_size, cuda, bidirectional):
    ...
    encoder = EncoderRNN(len(config.tokenizer.vocab), config.pad_size, hidden_size,
                         dropout_p=0.2, input_dropout_p=0, bidirectional=bidirectional, variable_lengths=True)
...
def main():
    seq2seq, optimizer,scheduler, train_dataloader, val_dataloader, test_dataloader, loss_fun, config = build(256, 64, True, True)
