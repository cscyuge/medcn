...
def main():
    model, optimizer, scheduler, train_dataloader, val_dataloader, test_dataloader, loss_fun, config = build(768, 48,
                                                                                                               True)
    ...
    for token_ids_src, _, seq_len_src, mask_src, token_ids_tar, __, seq_len_tar, mask_tar, tar_txt in tqdm(test_data):
        ...
        if len(src_ids) > 0:
            ...
            seq_len_srcs = len(src_ids[0]) - np.sum(src_ids == 0, axis=1)
